...
/*
std::lock()	 和 	std::lack_guard	 能组成简单的锁覆盖大多数情况,
但是有时需要更多的灵活性。在这些情况,可以使用标准库提供的std::unique_lock模板。

unique_lock实现
参考网址：
 http://llvm.org/docs/doxygen/html/UniqueLock_8h_source.html
...
std::lock_guard/std::unique_lock/std::shared_lock类模板在构造时是否加锁是可选的，C++11提供了3种加锁策略。
...
没有上锁:defer_lock:Do not lock on construction (and assume it is not already locked by thread)
           ...
           构造unique_lock之前,mutex没有被锁住。构造之后也不锁住。
           在进入临界区之前user需要调用unique_lock.lock();析构时解锁。
...
已经上锁:adopt_lock:Adopt current lock (assume it is already locked by thread).
           ...
           互斥锁之前已经上锁。unique_lock对象不需要再上锁。
           只在unique_lock对象销毁时自动对mutex解锁。
 * */
...
public:
    ...
    friend void swap(X& lhs, X& rhs)
    {
        if(&lhs==&rhs)
            return;
/*unique_lock是write lock。
 * 被锁后不允许其他线程执行被shared_lock或unique_lock的代码。  std::defer_lock不请求加锁.
 * 因此lock_b可被unique_lock的lock(),try_lock,unlock()函数获取,或者传递给std::lock()
*/
...
/*
std::unique_lock的灵活性允许对象在销毁之前放弃其拥有的锁。
可以使用unlock()来做这件事.std::unique_lock的成员函数(lock,unlock)提供类似于锁定和解锁互斥量的功能。
std::unique_lock对象在销毁前释放锁的能力使得：当锁没有必要在持有的时候,
可以在特定的代码分支对其进行选择性的释放。这对于应用性能来说很重要,因为持有锁的时间增加,
会导致性能下降,其他线程会等待这个锁的释放。
 */
