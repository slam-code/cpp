...
//#include <boost/thread/shared_mutex.hpp> //boost
...
unique_lock是write lock。被锁后不允许其他线程执行被shared_lock或unique_lock的代码。
 ...
 boost::shared_lock 类型的非独占锁可以在线程只对某个资源【读】访问的情况下使用。

 若一个线程修改的资源需要【写】访问，则需要一个独占锁。

 这样做也很明显：只需要【读】访问的线程不需要知道同一时间其他线程是否访问。
 因此非独占锁可以共享一个互斥体。
 * */
//不支持c++17:#define  shared_mutex shared_timed_mutex
...
{
    std::cout << "3.13 boost or c++17 ok ";
}

/*http://en.cppreference.com/w/cpp/thread/shared_lock
 * 使用std::mutex来保护dns的数据结构,这的确有些反应过度,
...
/*比起使用std::mutex实例进行同步,不如使用boost::shared_mutex来做同步。对于更新操作,可以使
 ...
 锁;同样的,当任一线程拥有一个独占锁是,其他线程就无法获得共享锁或独占锁,直到第 一个线程放弃其拥有的锁。*/
...
unique_lock是write lock。被锁后不允许其他线程执行被shared_lock或unique_lock的代码。在写操作时一般用这个。
可以同时限制unique_lock的写和share_lock的读。
 */